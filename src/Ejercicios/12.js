function getMinJump(obstacles) {
  ...
  // console.log(Math.max(...obstacles))
  for(let jump =2; jump<Math.max(...obstacles); jump++){
    if(obstacles.every(elem => elem%jump!==0)){
      return jump;
    }}

  return true
}
console.log(getMinJump([2, 4, 6, 8, 10])) // -> 4
// getMinJump([3, 7, 5]) // -> 2
// getMinJump([9, 5, 1]) // -> 2
...
// El Keanu Relfes 🧝 nos ha preparado una lista de obstáculos a evitar.El problema es que nos ha dado la lista
// de posiciones de los obstáculos desordenada... 😅 aunque al menos nunca la posición 0 puede tener un obstáculo.
